...
impl core::fmt::Display for CalendarError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            CalendarError::OutOfRange(msg) => write!(f, "Date out of supported range: {}", msg),
            CalendarError::InvalidParameters(msg) => write!(f, "Invalid calendar parameters: {}", msg),
            CalendarError::NotImplemented(msg) => write!(f, "Feature not yet implemented: {}", msg),
            CalendarError::ArithmeticError(msg) => write!(f, "Arithmetic error: {}", msg),
        }
    }
}
...
pub fn jdn_to_gregorian(jdn: JDN) -> (i32, u8, u8) {
    ...
    if jdn < i32::MIN as JDN || jdn > i32::MAX as JDN {
        panic!("JDN {} is outside supported range for Gregorian conversion ({} to {})", 
               jdn, i32::MIN, i32::MAX);
    }
    ...
    debug_assert!(day >= 1 && day <= 31, "Invalid day calculation: {}", day);
    debug_assert!(month >= 1 && month <= 12, "Invalid month calculation: {}", month);
...
mod tests {
    ...
    fn test_jdn_to_gregorian_sultan_agung_epoch() {
        ...
        let (year, month, day) = jdn_to_gregorian(2317690);
    ...
    fn test_round_trip_conversions() {
        ...
        for (year, month, day) in test_dates {
            ...
            assert_eq!((year, month, day), (year2, month2, day2), 
    ...
    fn test_calendar_error_display() {
        ...
        assert_eq!(error.to_string(), "Invalid calendar parameters: test params");
        
        let error = CalendarError::NotImplemented("test feature".to_string());
        assert_eq!(error.to_string(), "Feature not yet implemented: test feature");
