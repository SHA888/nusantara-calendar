...
use calendar_core::{CalendarDate, CalendarError, CalendarMetadata, HasAuspiciousness, JDN};
...
impl BalineseDate {
    ...
    pub fn from_ymd(year: i32, month: u8, day: u8) -> Result<Self, BalineseCalendarError> {
        ...
        if year < Self::MIN_GREGORIAN_YEAR || year > Self::MAX_GREGORIAN_YEAR {
            return Err(BalineseCalendarError::OutOfRange(format!(
                "Year {} is outside supported range {}-{}",
                year,
                Self::MIN_GREGORIAN_YEAR,
                Self::MAX_GREGORIAN_YEAR
            )));
        }

        if month < 1 || month > 12 {
            return Err(BalineseCalendarError::InvalidGregorianDate(format!(
                "Month {} is invalid (must be 1-12)",
                month
            )));
        }
        ...
        if day < 1 || day > max_day {
            return Err(BalineseCalendarError::InvalidGregorianDate(format!(
                "Day {} is invalid for month {} in year {} (max: {})",
    ...
    fn compute_sasih(jdn: JDN) -> (i32, Sasih, bool) {
        ...
        let adjusted_saka_year = if jdn >= 2_461_119 {
            // March 19, 2026 onwards
        ...
        let sasih_cycle = if jdn >= 2_461_119 {
            ((jdn - 2_461_119) % 360) as usize
        } else {
            (360 - ((2_461_119 - jdn) % 360) as usize) % 360
        };
        let sasih_index = (sasih_cycle / 30) % 12;
        let sasih = Sasih::from_index(sasih_index);

        // Nampih (intercalary month) detection (simplified)
        // Note: Real Nampih detection requires astronomical calculations and complex
        // traditional Balinese calendar rules. This is a basic approximation.
    ...
    fn validate_range(&self) -> Result<(), CalendarError> {
            ...
            || self.gregorian_year > Self::MAX_GREGORIAN_YEAR
        {
            return Err(CalendarError::OutOfRange(format!(
                "Gregorian year {} is outside supported range {}-{}",
                self.gregorian_year,
                Self::MIN_GREGORIAN_YEAR,
                Self::MAX_GREGORIAN_YEAR
            )));
...
impl HasAuspiciousness for BalineseDate {
    ...
    fn is_auspicious_day(&self) -> bool {
        ...
        let saptawara_auspicious = matches!(
            self.saptawara,
            Saptawara::Buda | Saptawara::Sukra | Saptawara::Wraspati
        );
        let lunar_auspicious = self.is_purnama && !self.is_tilem;
        let wuku_auspicious = matches!(self.wuku, Wuku::Sinta | Wuku::Landep | Wuku::Ukir); // Example auspicious Wuku
